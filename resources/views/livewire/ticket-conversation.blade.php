<div class="space-y-6">
    ...
    @if($pinnedNotes->isNotEmpty())
        <div class="rounded-lg border border-warning-300 bg-warning-50 dark:border-warning-600 dark:bg-warning-900/20 p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h4 class="text-sm font-semibold text-warning-700 dark:text-warning-400">Pinned Notes</h4>
            </div>
            <div class="space-y-2">
                @foreach($pinnedNotes as $note)
                    <div class="text-sm text-warning-800 dark:text-warning-300">
                        <span class="font-medium">{{ $note->author?->name ?? 'System' }}</span>
    ...
    <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        @if($reply->is_internal_note)
                            <button
                                ...
                                title="{{ $reply->is_pinned ? 'Unpin' : 'Pin' }}"
                            >
                                @if($reply->is_pinned)
        ...
        @empty
            <div class="text-center py-8 text-gray-500 dark:text-gray-400">
                ...
                <p class="text-sm">No replies yet. Start the conversation below.</p>
            </div>
        @endforelse
    </div>
    ...
    <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4">
        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
            {{ $this->isInternalNote ? 'Add Internal Note' : 'Reply' }}
        </h4>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    @if($this->isInternalNote)
                        ...
                        <span>This note is only visible to agents</span>
                    @else
                        <x-heroicon-o-globe-alt class="h-3.5 w-3.5 text-gray-400" />
                        <span>This reply will be visible to the customer</span>
                    @endif
                </div>
                ...
                >
                    ...
                    {{ $this->isInternalNote ? 'Add Note' : 'Send Reply' }}
                </button>
            </div>
        </form>
    </div>
